...
from utils import PAMR, affinity_propagation
...
class CLIPForSegmentation(BaseSegmentor):
    def __init__(self, clip_path, name_path, device=torch.device('cuda'),
                    ...
                    slide_stride=112, slide_crop=224, residual=False, empty_text=False, area_thd=None, text_t=0.25, thre=2.2):
        ...
        with torch.no_grad():
            for qw in query_words:
                ...
                if self.empty_text:
                    r_query = clip.tokenize([temp("nothing") for temp in openai_imagenet_template]).to(device)
                    r_feature = self.net.encode_text(r_query)
                    feature = feature - text_t * r_feature
    ...
    def forward_feature(self, img, logit_size=None):
        ...
        if self.slide_crop == 0:
            device = img.device
            img = _transform1(224)(img).to(device)

        image_features, attn_weights = self.net.encode_image(img, return_all=True, csa=True, residual=self.residual)
        ...
        quantile_thres = torch.quantile(attn_weights.float(), q=0.70, dim=1, keepdim=True)
        attn_cnt = (attn_weights > quantile_thres).sum(dim=0).float()
        attn_threshold = torch.median(attn_cnt) + attn_cnt.std() + self.thre  
        attn_mask = attn_cnt >= attn_threshold

        attn_final = attn_weights.mean(dim=0) * attn_mask
        attn_final = attn_final.to(logits.dtype)

        logits = affinity_propagation(logits, attn_final.unsqueeze(0))
    ...
    def forward_slide(self, img, img_metas, gt_masks, stride=112, crop_size=224):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                crop_gt = gt_masks[:, :, y1:y2, x1:x2]
                # save_and_show_tensor_as_image(crop_img)
    ...
    def predict(self, inputs, data_samples):
        ...
        if data_samples is not None:
            ...
            gt_masks = [data_sample.gt_sem_seg.data for data_sample in data_samples]
            gt_masks = torch.stack(gt_masks, dim=0)
            gt_masks = F.interpolate(gt_masks.float(), inputs.shape[2:], mode='nearest').long()
        else:
            batch_img_metas = [
                dict(
        ...
        if self.slide_crop > 0:
            seg_logits = self.forward_slide(inputs, batch_img_metas, gt_masks, self.slide_stride, self.slide_crop)
        else:
